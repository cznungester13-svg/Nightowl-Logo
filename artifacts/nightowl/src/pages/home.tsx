...
export function Home() {
  ...
  useEffect(() => {
    ...
    if (!pageViewTracked.current) {
      createAnalyticsEvent.mutate({ data: { type: 'page_view' as AnalyticsEventType } });
      pageViewTracked.current = true;
    }
  }, [createAnalyticsEvent]);

  const trackCtaClick = (label: string) => {
    createAnalyticsEvent.mutate({ data: { type: 'cta_click' as AnalyticsEventType, label } });
  };
    ...
    <div className="nightowl-page noise" id="top">
      <header className="absolute left-0 right-0 top-0 z-40 text-[#F7F2E8]">
        <div className="nightowl-shell flex h-[78px] items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-4">
            <Link href="/admin" className="text-[13px] font-medium text-[#d8e2dd] hover:text-white" data-testid="link-login">Log In</Link>
            <button type="button" onClick={goToContact} className="rounded-full bg-[#F7F2E8] px-5 py-2.5 text-[13px] font-bold text-[#273149] transition-transform hover:-translate-y-0.5" data-testid="button-header-contact">
        ...
        {menuOpen && (
          <nav className="mx-3 rounded-2xl border border-[#526168] bg-[#273149] p-4 shadow-xl md:hidden" aria-label="Mobile navigation">
            ...
            <Link href="/admin" onClick={() => setMenuOpen(false)} className="block border-b border-[#526168] px-3 py-3 text-sm text-[#e7eee9]">Log In</Link>
          ...
          <div className="nightowl-shell relative grid min-h-[720px] items-center gap-12 pb-16 pt-32 md:grid-cols-[1.05fr_.95fr] md:pb-20 md:pt-24">
            <div className="max-w-[610px]">
              ...
              <h1 className="reveal reveal-delay-1 font-display text-[clamp(3.5rem,8vw,7.4rem)] font-semibold leading-[.91] tracking-[-.075em] text-[#F7F2E8]">
                {settings?.heroTitle ? (
                  <span>{settings.heroTitle.split('\n').map((line, i) => <span key={i}>{line}<br /></span>)}</span>
                ) : (
                  <>Your business<br /></>
                )}
                <span className="text-[#9ed3ca]">{settings?.heroAccent || "never sleeps."}</span>
              </h1>
              <p className="reveal reveal-delay-2 mt-7 max-w-[480px] text-[17px] leading-7 text-[#c5d0ca]">
                {settings?.heroDescription || "NightOwl handles the work that keeps you up — so you can finally switch off without your business going quiet."}
              </p>
      ...
      <footer className="bg-[#273149] py-10 text-[#F7F2E8]">
        <div className="nightowl-shell flex flex-col gap-8 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-3 text-xs text-[#b9c8c1]"><a href="#capabilities" data-testid="footer-link-capabilities">Capabilities</a><a href="#pricing" data-testid="footer-link-pricing">Pricing</a><a href="#contact" data-testid="footer-link-contact">Contact</a><Link href="/admin" data-testid="footer-link-admin">Admin</Link><a href={`mailto:${settings?.contactEmail || "hello@nightowl.work"}`} aria-label="Email NightOwl" data-testid="footer-email"><Mail size={16} /></a><a href="https://www.linkedin.com" target="_blank" rel="noreferrer" aria-label="NightOwl on LinkedIn" data-testid="footer-linkedin"><Linkedin size={16} /></a></div>
          <p className="text-[10px] uppercase tracking-[.14em] text-[#71817e]">© {new Date().getFullYear()} NightOwl</p>
        </div>
      </footer>
    </div>
  );
